...
@interface XFSphereView()<UIGestureRecognizerDelegate>
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    XFPoint normalDirection;
    CGPoint last;
...
- (void)setItems:(NSArray *)items
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        XFPoint point = XFPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(XFPoint)];
    ...
    normalDirection = XFPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(XFPoint)direction andAngle:(CGFloat)angle
{
    ...
    XFPoint rPoint = XFPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(XFPoint)];
...
- (void)setTagOfPoint: (XFPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        XFPoint direction = XFPointMake(last.y - current.y, current.x - last.x, 0);
